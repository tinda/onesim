 ...
 * <CODE>MULTILINESTRING</CODE>s. Skips all <CODE>POINT</CODE> data. Other data
 * causes IOException.
 */
public class WKTMapReader extends WKTReader {
	 ...
	 * 
	 * @param bidi
	 *            If true, all paths are set bidirectional (false -> not)
	 */
	 ...
	 * 
	 * @param file
	 *            The file where the WKT data is read from
	 * @param type
	 *            The type to use (integer value, see class {@link MapNode}))
	 * @throws IOException
	...
	/**
	 * Add paths to current path set. Adding paths multiple times has the same
	 * result as concatenating the data before adding it.
	 * 
	 * @param input
	 *            Reader where the WKT data is read from
	 * @param nodeType
	 *            The type to use (integer value, see class {@link MapNode}))
	 * @throws IOException
	...
	public void addPaths(Reader input, int nodeType) throws IOException {
		...
		while ((type = nextType()) != null) {
			...
			} else if (type.equals(LINESTRING1)) {
				contents = readNestedContents();
				updateMap(parseLineString(contents));
			} else if (type.equals(LINESTRING2)) {
				contents = readNestedContents();
				updateMap(parseLineString(contents));
			} else if (type.equals(LINESTRING3)) {
				contents = readNestedContents();
				updateMap(parseLineString(contents));
			} else if (type.equals(LINESTRING4)) {
				contents = readNestedContents();
				updateMap(parseLineString(contents));
			} else {
	 ...
	 * 
	 * @param c
	 *            The location coordinates of the node
	 * @param previous
	 *            Previous node whose neighbor node at c is
