...
public abstract class Connection {
	...
	public void finalizeTransfer() {
		...
		getOtherNode(msgFromNode).messageTransferred(this.msgOnFly.getId(), msgFromNode);
		clearMsgOnFly();
	}
	...
	public int getTotalBytesTransferred() {
		if (this.msgOnFly == null) {
			...
			if (isMessageTransferred()) {
				...
				return this.bytesTransferred + (msgOnFly.getSize() - getRemainingByteCount());
			}
		}
	}
	...
	public String toString() {
		return fromNode + "<->" + toNode + " (" + getSpeed() + "Bps) is " + (isUp() ? "up" : "down") + (this.msgOnFly != null ? " transferring " + this.msgOnFly + " from " + this.msgFromNode : "");
	}
